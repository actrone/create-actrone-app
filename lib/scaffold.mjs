...
/** Human-readable display name for each framework slug, used in generated docs. */
const FRAMEWORK_LABELS = {
  core: "the library on its own",
...
// Recall is by shared keywords; \`npm install fastembed\` switches it to recall by meaning.
// When Actrone's hosted platform launches, swapping MemoryManager for its drop-in
// ActroneMemoryManager moves this to governed, hosted memory with no other change.
async function main() {
...
Plug in the Redis + Qdrant adapters for a durable self-hosted backend, and install
\`fastembed\` for recall by meaning with a local model. When Actrone's hosted platform
launches, swapping \`MemoryManager\` for its drop-in \`ActroneMemoryManager\` moves you to
governed, hosted memory. See the docs at https://actrone.com/docs/memory/overview.
`;
}
